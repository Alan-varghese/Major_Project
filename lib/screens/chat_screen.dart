...
// To store the user's avatar URL
  bool _isMalayalam = false; // To track language preference
  ...
  Future<void> _fetchUserData() async {
    final user = Supabase
        .instance.client.auth.currentUser; // Replace with your auth method
    if (user != null) {
      setState(() {
// Get user's name
  ...
  Future<void> _sendMessage(String message) async {
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          'message': message,
          'language': _isMalayalam
              ? 'ur'
              : 'en', // Add language parameter based on toggle
        }),
      );
    ...
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        try {
          ...
          print(
              'File details: ${request.files.first.filename}, ${request.files.first.length} bytes');
      ...
      appBar: AppBar(
        ...
        leading: PopupMenuButton<int>(
          icon: const Icon(Icons.account_circle_rounded,
              color: Colors.white), // Use an icon for the dropdown
          onSelected: (value) {
            if (value == 1) {
            ...
            PopupMenuItem(
              ...
              child: Row(
                ...
                children: [
                  const Text("English",
                      style: TextStyle(
                          fontSize: 14)), // Left side label (Malayalam)
                  Switch(
                    value: _isMalayalam,
                    onChanged: (value) {
                      setState(() {
                        _isMalayalam = value; // Update the language preference
                      });
                  ...
                  const Text("Malayalam",
                      style: TextStyle(
                          fontSize: 14)), // Right side label (English)
                ],
              ),
            ),
      ...
      body: Container(
        decoration: BoxDecoration(
          image: DecorationImage(
            ...
            opacity:
                0.2, // Optional: control the opacity of the background image
          ),
        ),
        child: LayoutBuilder(
...
class Bubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    bool isPdfMessage =
        message.startsWith('PDF: '); // Check if the message is a PDF
